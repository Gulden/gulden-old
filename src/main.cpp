...
// Copyright (c) 2013-2100 GuldenCoin developers
...
uint256 hashGenesisBlock("0x6c5d71a461b5bff6742bb62e5be53978b8dec5103ce52d1aaab8c6a251582f92");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Guldencoin: starting difficulty is 1 / 2^12
...
{
    int64 nSubsidy = 1000 * COIN; // 1000 Gulden for each block
...
unsigned int static GetNextWorkRequired_original(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Guldencoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired_KGW(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
       static const int64 BlocksTargetSpacing = .5 * 60;
			int64 NewBlocksTargetSpacing = .5*60;
			if (pindexLast->nHeight > 30000) NewBlocksTargetSpacing = 150; // 150 seconds
       unsigned int TimeDaySeconds = 60 * 60 * 24;

       int64 PastSecondsMin = TimeDaySeconds * 0.01;
			if (pindexLast->nHeight > 33300) PastSecondsMin = TimeDaySeconds * 0.25;
       int64 PastSecondsMax = TimeDaySeconds * 0.14;
			if (pindexLast->nHeight > 33300) PastSecondsMax = TimeDaySeconds * 7;
       uint64 PastBlocksMin = PastSecondsMin / BlocksTargetSpacing;
			if (pindexLast->nHeight > 33300) PastBlocksMin = PastSecondsMin / NewBlocksTargetSpacing;
       uint64 PastBlocksMax = PastSecondsMax / BlocksTargetSpacing;
			if (pindexLast->nHeight > 33300) PastBlocksMax = PastSecondsMax / NewBlocksTargetSpacing; 

       return KimotoGravityWell(pindexLast, pblock, NewBlocksTargetSpacing, PastBlocksMin, PastBlocksMax);
}

unsigned int static GetNextWorkRequired_DGW3(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Guldencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1009843200; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2200095;
        if (fTestNet)
        {
            block.nTime    = 1399759200;
            block.nNonce   = 332577;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x4bed0bcb3e6097445ae68d455137625bb66f0e7ba06d9db80290bf72e3d6dcf8"));
////
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Guldencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GuldencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GuldencoinMiner(CWallet *pwallet)
{
    printf("GuldencoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running GuldencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GuldencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GuldencoinMiner, pwallet));
}
